# Enhancement Build Plan 2 — Cluster-backed LLM generation (replaces OpenRouter)
...
("Cluster is safe... let's scrap the Open Router integration"). They are **not deleted** (no
...
## 1. What Cluster actually is, and what we found (grounded in your attachments — not guessed)

Your own research (`docs/CalpysoWebsites.md`, the live ATLAS dashboard listing) revealed there are
**three different things** all referred to as "Cluster," which is exactly the source of the
confusion you flagged:

1. `docs/XYZCluster.md` — a **generic** integration guide referencing `application=genllm` and
...
2. **`LLM-Cluster`** (under "Experiments," 4 CPU, **no GPU allocated at all**) — clicking Access
...
```
GET  {Cluster_BASE_URL}/cmd/state?application={app-name}   -> text body: STOP | STARTING | RUNNING | ONLINE | ERROR
GET  {Cluster_BASE_URL}/cmd/start?application={app-name}    -> triggers a start; takes ~4-5 minutes to reach ONLINE
POST {Cluster_BASE_URL}/{app-name}/v1/chat/completions      -> OpenAI-compatible chat completions
     body: { "model": "<model-id>", "messages": [...], "max_tokens": N }
```

`Cluster_BASE_URL` = `https://apps.services.Cluster.intra.chrysler.com` for both local dev and
XYZ-deployed apps (per `docs/XYZCluster.md`, confirmed identical for both environments). No
...
| Rank | App name | Model ID (exact string to send) | License | GPUs | Cluster's own use-case tag | Role in this build |
|---|---|---|---|---|---|---|
...
- `LLM-Cluster` (the Experiments-section app) — no usable contract, out of scope per §1.
...
OpenRouter-specific and is removed with it — Cluster has no equivalent concept (it's an internal
service, not a multi-provider router).

**Add:**

- `Cluster_BASE_URL` (env override, default `https://apps.services.Cluster.intra.chrysler.com`)
- `Cluster_MODEL_CANDIDATES` — hardcoded default array of the three `{app, model}` pairs from §2,
  in priority order, with an optional env-var override (`Cluster_MODEL_CANDIDATES` as a JSON
  string) for flexibility without a code change if the ranking ever needs to shift.
- `Cluster_CHAT_TIMEOUT_MS` (default 90000 — Cluster's own docs recommend 60-120s for chat
...
- `Cluster_STATE_TIMEOUT_MS` (default 10000 — state checks should be fast).
- **A scoped TLS-bypass helper, using only Node's built-in `node:https` module — no new npm
  dependency.** Cluster's cert is self-signed (confirmed: docs explicitly say `verify=False`
  ...
  *every* outbound HTTPS request made by the whole process, including any future integration —
  ...
  calls targeting `Cluster_BASE_URL`. This keeps the bypass scoped to exactly the one
  ...
  A small `ClusterRequest(path, { method, body, timeoutMs })` helper wraps this (promisified
...
- `callClusterChat(appName, modelId, messages, { responseFormat, maxTokens })` →
  1. `POST {Cluster_BASE_URL}/{appName}/v1/chat/completions` with `{ model: modelId, messages,
  ...
  4. **This structured-vs-prompt-based fallback must be verified empirically against the real
     Cluster endpoint early in implementation** (§7) — vLLM's OpenAI-compatible server generally
...
- `LlmUnavailableError` — same sentinel-error concept as before, now thrown when every candidate
  in `Cluster_MODEL_CANDIDATES` has been tried (or skipped because its state wasn't `ONLINE`) and
  none succeeded.
- **Per-candidate resilience loop** used by both `handleGapAnalysis`/`handleGenerate` (replaces the
  ...
  `Cluster_MODEL_CANDIDATES`, in order:
  1. `state = await getAppState(app)`.
  2. If `state === "ONLINE"`: attempt `callClusterChat(...)`. Success → return the result
...
**Startup log block**: replace the OpenRouter-specific log lines with equivalent Cluster ones
(e.g. `[llm] Cluster enabled - primary: vllm-glm-52, 2 fallback candidate(s) configured`).

### 3.2 `app/vite.config.ts` (dev — same duplication pattern as before, same justification)

`createLlmDevPlugin` gets the identical Cluster logic duplicated (not imported from `server.mjs`,
...
- **`app/src/App.tsx`**: add a `useEffect` on mount that (a) calls `triggerLlmWarmup()` once,
  ...
  perfectly functional) deterministic fallback while Cluster finishes starting up for next time.
...
**No API key is needed at all** — Cluster access is controlled purely by network location (the
XYZ pod already lives inside the same VPC, per `docs/XYZCluster.md`). This eliminates the
...
|---|---|---|
| `Cluster_BASE_URL` | No | `https://apps.services.Cluster.intra.chrysler.com` |
| `Cluster_MODEL_CANDIDATES` | No | the three-entry list in §2, as JSON |
| `Cluster_CHAT_TIMEOUT_MS` | No | `90000` |
| `Cluster_STATE_TIMEOUT_MS` | No | `10000` |
...
- **Live manual verification, required before calling this done** (mirrors the rigor already
  ...
  `apps.services.Cluster.intra.chrysler.com` (confirmed reachable identically from local dev and
  ...
  5. **Specifically confirm whether `response_format`/structured JSON output is honored** by at
     ...
     `callClusterChat` is a rarely-used safety net or the primary code path in practice. Either
...
- **Two real bugs found and fixed during live verification** (not present in this plan's original
  ...
  1. `generateSchema`'s JSON schema puts section names at the top level, but the frontend's
     ...
     Cluster's response was perfectly valid. This bug pre-dates Cluster (same shape existed in the
  ...
  2. The client-side `DEFAULT_TIMEOUT_MS` in `app/src/api/llmClient.ts` was still 20000 (inherited
     from the OpenRouter design) — shorter than the backend's own 90000ms Cluster timeout, and
     shorter than Cluster's real observed response times (12-37s). Bumped to 100000 so the client
  ...
  4. Also found: a single shared `max_tokens: 4096` default was an unnecessary regression vs.
     ...
     legitimately need more). Split into `Cluster_GAP_ANALYSIS_MAX_TOKENS` (1024) and
     `Cluster_GENERATE_MAX_TOKENS` (8192).
  5. Also found, via a safe controlled test (temporary `Cluster_MODEL_CANDIDATES` env-var
     override with fake app names - no real Cluster app was ever touched/disabled): the
...
- **Cold-start UX**: the warm-up banner design (§3.3) means a user's very first visit after a
  ...
  spinner.
- **Self-signed certificate handling**: scoped to `node:https` + a dedicated `Agent` for Cluster
...
- No integration with `LLM-Cluster`, `middlewareai-*` apps, or `kvllm-gpt-oss-120b` — all
